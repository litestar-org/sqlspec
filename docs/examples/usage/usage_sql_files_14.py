...
async def test_multi_database_setup_example(tmp_path: Path, postgres_service: PostgresService) -> None:
    ...
    async with spec.provide_session(postgres_config) as pg_session:
        await pg_session.execute("""CREATE TABLE users ( id INTEGER PRIMARY KEY, username TEXT, email TEXT)""")
        await pg_session.execute(
            """ INSERT INTO users (id, username, email) VALUES (1, 'old_name', 'old@example.com');"""
        )

        await pg_session.execute(pg_query, **params)

    with spec.provide_session(sqlite_config) as sqlite_session:
        sqlite_session.execute("""CREATE TABLE users ( id INTEGER PRIMARY KEY, username TEXT, email TEXT)""")
        sqlite_session.execute(""" INSERT INTO users (id, username, email) VALUES (1, 'john_doe', 'jd@example.com');""")
