...
def test_binding_multiple_configs() -> None:
    ...
    with tempfile.NamedTemporaryFile(suffix=".db", delete=True) as tmp:
        ...
        sqlite_config = db_manager.add_config(SqliteConfig(connection_config={"database": tmp.name}))
        dsn = os.getenv("SQLSPEC_USAGE_PG_DSN", "postgresql://localhost/db")
        pg_config = db_manager.add_config(AsyncpgConfig(connection_config={"dsn": dsn}))
        ...
        # end-example
        assert sqlite_config.connection_config["database"] == tmp.name
        assert pg_config.connection_config["dsn"] == os.getenv("SQLSPEC_USAGE_PG_DSN", "postgresql://localhost/db")
