...
* Data dictionary column metadata reports more about each column.
  ...
  sequence owned by a serial or identity column on PostgreSQL and CockroachDB, and
  ``column_type`` and ``column_key`` for a single MySQL table. On PostgreSQL and
  CockroachDB, ``get_columns(table=...)`` without a schema now finds the table
  through the session search path instead of assuming ``public``.
