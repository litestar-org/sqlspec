...
def test_plan_squash_mixed_types_plan_squash_all_sql_returns_single_plan(tmp_path: Path) -> None:
...
def test_plan_squash_mixed_types_plan_squash_all_python_returns_single_plan(tmp_path: Path) -> None:
...
def test_plan_squash_mixed_types_plan_squash_mixed_returns_multiple_plans(tmp_path: Path) -> None:
    ...
    assert plans[1].target_version == "0002"
    assert plans[1].target_path.suffix == ".py"
    assert len(plans[1].source_migrations) == 2
    assert plans[2].target_version == "0003"
    assert plans[2].target_path.suffix == ".sql"
    assert len(plans[2].source_migrations) == 1


def test_plan_squash_mixed_types_plan_squash_mixed_version_numbering(tmp_path: Path) -> None:
...
def test_plan_squash_mixed_types_plan_squash_mixed_preserves_source_versions(tmp_path: Path) -> None:
