...
def test_version_rich_comparison_migration_version_explicit_gt_and_ge() -> None:
    """Greater-than comparisons should not rely on reflected fallback methods."""
    v1 = parse_version("0001")
    v2 = parse_version("0002")
    assert v2 > v1
    assert v2 >= v1
...
def test_version_rich_comparison_migration_version_gt_and_ge_cover_timestamp_and_extension_versions() -> None:
    """The explicit rich methods should preserve the existing mixed ordering rules."""
    sequential = parse_version("9999")
    timestamp = parse_version("20200101000000")
    ext_aaa = parse_version("ext_aaa_0001")
    ext_bbb = parse_version("ext_bbb_0001")
    assert timestamp > sequential
    assert timestamp >= sequential
    assert ext_bbb > ext_aaa
    assert ext_bbb >= ext_aaa


def test_version_rich_comparison_migration_version_final_decorator_preserves_dataclass_behavior() -> None:
    """Adding @final should not change frozen dataclass copy or pickle behavior."""
    version = parse_version("ext_litestar_0001")
