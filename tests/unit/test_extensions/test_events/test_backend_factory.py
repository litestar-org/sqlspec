...
from sqlspec.adapters.asyncpg.events.backend import create_event_backend as asyncpg_factory
...
def test_asyncpg_hybrid_backend_factory() -> None:
    config = AsyncpgConfig(connection_config={"dsn": "postgresql://example"}, extension_config={"events": {}})
...
def test_psycopg_listen_notify_backends() -> None:
    async_config = PsycopgAsyncConfig(connection_config={"dbname": "example"})
    sync_config = PsycopgSyncConfig(connection_config={"dbname": "example"})
...
def test_psqlpy_backends() -> None:
    config = PsqlpyConfig(connection_config={"dsn": "postgresql://example"}, extension_config={"events": {}})
