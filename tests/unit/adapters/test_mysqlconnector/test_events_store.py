...
"""Unit tests for mysql-connector event queue store index DDL (DD-based existence)."""
...
def test_mysql_events_index_check_no_information_schema_string() -> None:
    ...
    from sqlspec.adapters.mysqlconnector.events.store import MysqlConnectorSyncEventQueueStore

    store = MysqlConnectorSyncEventQueueStore(
        MysqlConnectorSyncConfig(connection_config={"host": "localhost", "database": "test"})
    )
