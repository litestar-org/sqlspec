...
@pytest.mark.anyio
async def test_overwrite_issues_delete_before_insert() -> None:
    conn = _FakeConnection(in_transaction=False)
    await _driver(conn).load_from_arrow("t", pa.table({"a": [1, 2]}), overwrite=True)
...
@pytest.mark.anyio
async def test_rollback_on_error_when_owning_transaction() -> None:
