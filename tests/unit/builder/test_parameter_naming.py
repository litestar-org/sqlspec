...
from sqlspec.builder import _parsing_utils, _select, parse_condition_expression
...
def test_mixed_parameter_types_preserve_names() -> None:
    ...
    query = (
        ...
        .set({"username": "john_doe", "age": 28, "salary": 75000.5, "is_active": True, "last_seen": None})
    ...
    assert 75000.5 in params.values()
...
def test_parameter_validator_singleton_parsing_utils_exposes_parameter_validator_singleton() -> None:
    assert isinstance(_parsing_utils._PARAMETER_VALIDATOR, ParameterValidator)


class CountingValidator:
    """Patchable stand-in for the singleton validator."""

    def __init__(self, original: ParameterValidator) -> None:
        self.calls = 0
        self.original = original

    def extract_parameters(self, condition: str):
        self.calls += 1
        return self.original.extract_parameters(condition)


def test_parameter_validator_singleton_parse_condition_expression_uses_parameter_validator_singleton(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    validator = CountingValidator(_parsing_utils._PARAMETER_VALIDATOR)
    monkeypatch.setattr(_parsing_utils, "_PARAMETER_VALIDATOR", validator)
    _parsing_utils.parse_condition_expression("id = :id")
    assert validator.calls == 1


def test_parameter_validator_singleton_select_where_uses_parameter_validator_singleton(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    validator = CountingValidator(_parsing_utils._PARAMETER_VALIDATOR)
    monkeypatch.setattr(_select, "_PARAMETER_VALIDATOR", validator)
    sql.select("*").from_("users").where("id = :id", id=1)
    assert validator.calls == 1


def test_parameter_validator_singleton_parameter_validator_singleton_reuses_scan_cache() -> None:
    first = _parsing_utils._PARAMETER_VALIDATOR.extract_parameters("id = :id")
    second = _parsing_utils._PARAMETER_VALIDATOR.extract_parameters("id = :id")
    assert second == first
