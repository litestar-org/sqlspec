...
def test_raw_with_named_parameters_returns_sql_object() -> None:
    ...
    assert stmt.raw_sql == "name = :name_param"
...
def test_raw_with_multiple_named_parameters() -> None:
    ...
    assert isinstance(stmt, SQL)
    assert stmt.raw_sql == "price BETWEEN :min_price AND :max_price"
...
def test_raw_with_complex_sql_and_parameters() -> None:
    ...
    assert isinstance(stmt, SQL)
    assert "LOWER(name) LIKE LOWER(:pattern)" in stmt.raw_sql
    assert "status = :status" in stmt.raw_sql
...
def test_raw_parameter_overwrite_behavior() -> None:
    ...
    assert isinstance(stmt, SQL)
    assert stmt.raw_sql.count(":value") == 2
...
def test_sql_call_with_update_returning() -> None:
    ...
    assert "UPDATE" in query.raw_sql.upper()
    assert "RETURNING" in query.raw_sql.upper()
...
def test_sql_call_with_insert_returning() -> None:
    ...
    assert isinstance(query, SQL)
    assert query.returns_rows()
    assert "INSERT" in query.raw_sql.upper()
    assert "RETURNING" in query.raw_sql.upper()
...
def test_sql_call_with_delete_returning() -> None:
    ...
    assert isinstance(query, SQL)
    assert query.returns_rows()
    assert "DELETE" in query.raw_sql.upper()
    assert "RETURNING" in query.raw_sql.upper()
...
def test_build_copy_to_statement_generates_expected_sql() -> None:
    ...
    rendered = statement.raw_sql
    assert (
        rendered == "COPY \"public.users\" TO 's3://bucket/output.parquet' WITH (FORMAT 'parquet', COMPRESSION 'gzip')"
    )
    expression = parse_one(rendered, read="postgres")
...
def test_builder_cache_key_cache_key_does_not_render_expression_sql(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fail_sql_render(*args: object, **kwargs: object) -> str:
        msg = "cache key must hash the expression structurally"
...
def test_native_layout_wave3_expression_wrappers_are_slotted_and_functional() -> None:
    """Expression wrappers should not allocate instance dicts after slotting."""
    wrappers = (
        ExpressionWrapper(exp.column("x")),
        AggregateExpression(exp.Count(this=exp.Star())),
        FunctionExpression(exp.Anonymous(this="fn", expressions=[])),
        MathExpression(exp.Round(this=exp.column("x"))),
        StringExpression(exp.Upper(this=exp.column("name"))),
        ConversionExpression(exp.Cast(this=exp.column("x"), to=exp.DataType(this=exp.DataType.Type.INT))),
    )
    for wrapper in wrappers:
        assert not hasattr(wrapper, "__dict__")
        assert isinstance(wrapper.expression, exp.Expression)
    alias = wrappers[1].as_("total")
    assert isinstance(alias, exp.Alias)
    assert alias.alias == "total"
...
def test_native_layout_wave3_join_builder_slots_remove_dead_condition_field() -> None:
    ...
    assert join.args.get("side") == "LEFT"
    fallback_builder = create_join_builder("RIGHT")
    assert not hasattr(fallback_builder, "_condition")


def test_native_layout_wave3_explain_slots_drop_dead_statement_reference() -> None:
    """Explain should keep resolved SQL without retaining the original statement object."""
    explain = Explain("SELECT 1", dialect="postgres")
    assert not hasattr(explain, "__dict__")
    assert not hasattr(explain, "_statement")
    assert explain._statement_sql == "SELECT 1"
    built = explain.build()
