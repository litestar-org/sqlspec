...
def test_dtype_to_array_code_mapping() -> None:
    ...
    from sqlspec.adapters.oracledb._numpy_handlers import DTYPE_TO_ARRAY_CODE
...
def test_input_type_handler_registers_numpy_converter() -> None:
    ...
    result = _input_type_handler(mock_cursor, arr, 1)
...
def test_output_type_handler_registers_numpy_converter() -> None:
    ...
    result = _output_type_handler(mock_cursor, mock_metadata)
...
def test_register_numpy_handlers_with_numpy_not_installed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import sqlspec.adapters.oracledb._numpy_handlers

    monkeypatch.setattr(sqlspec.adapters.oracledb._numpy_handlers, "NUMPY_INSTALLED", False)
