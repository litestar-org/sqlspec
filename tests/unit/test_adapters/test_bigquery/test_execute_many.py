...
def test_is_simple_insert_operation_basic_insert() -> None:
    ...
    from sqlspec.adapters.bigquery.core import is_simple_bigquery_insert

    assert is_simple_bigquery_insert("INSERT INTO test (a, b) VALUES (1, 2)")
...
def test_is_simple_insert_operation_with_named_params() -> None:
    ...
    assert is_simple_bigquery_insert("INSERT INTO test (a, b) VALUES (@a, @b)")
...
def test_is_simple_insert_operation_not_insert() -> None:
    ...
    assert not is_simple_bigquery_insert("UPDATE test SET a = 1")
    assert not is_simple_bigquery_insert("DELETE FROM test WHERE a = 1")
    assert not is_simple_bigquery_insert("SELECT * FROM test")
...
def test_is_simple_insert_operation_insert_select() -> None:
    ...
    result = is_simple_bigquery_insert("INSERT INTO test SELECT * FROM other")
...
def test_is_simple_insert_operation_malformed_sql() -> None:
    ...
    assert not is_simple_bigquery_insert("NOT VALID SQL AT ALL")


def test_extract_table_from_insert_simple() -> None:
    """Test extracting table name from simple INSERT."""
    from sqlspec.adapters.bigquery.core import extract_bigquery_insert_table

    assert extract_bigquery_insert_table("INSERT INTO test (a) VALUES (1)") == "test"
...
def test_extract_table_from_insert_qualified() -> None:
    ...
    result = extract_bigquery_insert_table("INSERT INTO project.dataset.table (a) VALUES (1)")
...
def test_extract_table_from_insert_not_insert() -> None:
    ...
    assert extract_bigquery_insert_table("SELECT * FROM test") is None
...
def test_extract_table_from_insert_malformed() -> None:
    ...
    assert extract_bigquery_insert_table("NOT VALID SQL") is None
