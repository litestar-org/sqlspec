...
class TestSQLiteStorageOperations:
    ...
    def test_export_to_storage_parquet(self, sqlite_driver: SqliteDriver, tmp_path: Any) -> None:
        ...
        sqlite_driver._resolve_backend_and_path = MagicMock(
    ...
    def test_storage_operations_with_connection_override(self, sqlite_driver: SqliteDriver) -> None:
        ...
        statement = SQL("SELECT * FROM users")
        sqlite_driver.fetch_arrow_table(statement, connection=override_conn)
    ...
    def test_ingest_arrow_table_with_mode_replace(
        self, sqlite_driver: SqliteDriver, mock_sqlite_connection: MagicMock
    ) -> None:
