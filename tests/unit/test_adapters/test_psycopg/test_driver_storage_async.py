...
class TestPsycopgAsyncStorageOperations:
    ...
    @pytest.mark.asyncio
    async def test_ingest_arrow_table(self, psycopg_async_driver: PsycopgAsyncDriver, mock_psycopg_async_connection: AsyncMock) -> None:
    ...
    async def test_export_to_storage_parquet(self, psycopg_async_driver: PsycopgAsyncDriver, tmp_path: Any) -> None:
        ...
        psycopg_async_driver._resolve_backend_and_path = MagicMock(return_value=(mock_backend, str(tmp_path / "output.parquet")))
    ...
    async def test_import_from_storage_parquet(self, psycopg_async_driver: PsycopgAsyncDriver, tmp_path: Any) -> None:
        ...
        # Patch the _resolve_backend_and_path method to return our mock
        psycopg_async_driver._resolve_backend_and_path = MagicMock(return_value=(mock_backend, str(tmp_path / "input.parquet")))
    ...
    async def test_fetch_arrow_table_with_filters(self, psycopg_async_driver: PsycopgAsyncDriver) -> None:
        ...
        statement = SQL("SELECT * FROM users")
        result = await psycopg_async_driver.fetch_arrow_table(statement, None, active_filter)
    ...
    async def test_storage_operations_with_connection_override(self, psycopg_async_driver: PsycopgAsyncDriver) -> None:
        ...
        statement = SQL("SELECT * FROM users")
        result = await psycopg_async_driver.fetch_arrow_table(statement, connection=override_conn)
