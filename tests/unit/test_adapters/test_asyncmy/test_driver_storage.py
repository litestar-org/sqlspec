...
class TestAsyncMyStorageOperations:
    ...
    async def test_export_to_storage_parquet(self, asyncmy_driver: AsyncmyDriver, tmp_path: Any) -> None:
        ...
        asyncmy_driver._resolve_backend_and_path = MagicMock(return_value=(mock_backend, str(tmp_path / "output.parquet")))
    ...
    async def test_import_from_storage_parquet(self, asyncmy_driver: AsyncmyDriver, tmp_path: Any) -> None:
        ...
        # Patch the _resolve_backend_and_path method to return our mock
        asyncmy_driver._resolve_backend_and_path = MagicMock(return_value=(mock_backend, str(tmp_path / "input.parquet")))
    ...
    async def test_fetch_arrow_table_with_filters(self, asyncmy_driver: AsyncmyDriver) -> None:
        ...
        statement = SQL("SELECT * FROM users")
        result = await asyncmy_driver.fetch_arrow_table(statement, None, active_filter)
    ...
    async def test_storage_operations_with_connection_override(self, asyncmy_driver: AsyncmyDriver) -> None:
        ...
        statement = SQL("SELECT * FROM users")
        result = await asyncmy_driver.fetch_arrow_table(statement, connection=override_conn)
    ...
    @pytest.mark.asyncio
    async def test_ingest_arrow_table_with_mode_replace(self, asyncmy_driver: AsyncmyDriver, mock_asyncmy_connection: AsyncMock) -> None:
