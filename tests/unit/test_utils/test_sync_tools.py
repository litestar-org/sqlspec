...
def test_run_await_async_integration() -> None:
    ...
    back_to_sync = run_(async_version)  # type: ignore[arg-type]
...
async def test_ensure_async_await_integration() -> None:
    ...
    sync_version = await_(sync_func, raise_sync_error=False)  # type: ignore[arg-type]
...
async def test_error_handling_in_complex_scenarios() -> None:
    ...
    sync_version = run_(async_version)  # type: ignore[arg-type]
