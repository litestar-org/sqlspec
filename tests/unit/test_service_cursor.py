...
async def test_walk_schema_and_single_query(
    ...
    page = await _call(service, "paginate_cursor", "SELECT id, v, n FROM items", CursorFilter(_KEYS, 10))
    assert len([sql for sql in statements if sql.startswith("SELECT")]) == 1
    forward = list(page.items)
    while page.next_cursor:
        page = await _call(
            service, "paginate_cursor", "SELECT id, v, n FROM items", CursorFilter(_KEYS, 10, page.next_cursor)
        )
    ...
    while page.previous_cursor:
        page = await _call(
            service, "paginate_cursor", "SELECT id, v, n FROM items", CursorFilter(_KEYS, 10, page.previous_cursor)
        )
    ...
    typed = await _call(
        service, "paginate_cursor", "SELECT id, v, n FROM items", CursorFilter(_KEYS, 10), schema_type=Item
    )
    ...
    next_typed = await _call(
        service,
        "paginate_cursor",
        "SELECT id, v, n FROM items",
        CursorFilter(_KEYS, 10, typed.next_cursor),
        schema_type=Item,
    )
...
def test_invalid_filters(sqlite_sync_driver: Any, filters: list[Any]) -> None:
    with pytest.raises(ImproperConfigurationError):
        SQLSpecSyncService(session=sqlite_sync_driver).paginate_cursor("SELECT id FROM users", *filters)
...
def test_constructor_conflicts(sqlite_sync_driver: Any, extra: Any) -> None:
    with pytest.raises(ImproperConfigurationError):
        SQLSpecSyncService(session=sqlite_sync_driver).paginate_cursor(
...
def test_cursor_runs_after_predicate(sqlite_sync_driver: Any) -> None:
    page = SQLSpecSyncService(session=sqlite_sync_driver).paginate_cursor(
...
def test_positional_base_parameters(sqlite_sync_driver: Any) -> None:
    ...
    page = service.paginate_cursor("SELECT id FROM users WHERE id > ?", 0, CursorFilter([CursorKey("id")], 1))
