"""Test parameter tracking in expression optimization."""

from sqlglot import parse_one

from sqlspec.statement.pipeline import SQLTransformContext, optimize_step
from sqlspec.statement.sql import SQL


def test_expression_simplifier_tracks_parameter_reordering() -> None:
    """Test that optimize step tracks when parameters are reordered."""
    ...
    context = SQLTransformContext(
        ...
        dialect="duckdb",
        parameters={"param_0": 200, "param_1": 400},  # Lower bound, upper bound
    )

    # Process with optimize step
    result_context = optimize_step(context)

    # Check if parameter mapping was created
    if "parameter_position_mapping" in result_context.metadata:
        mapping = result_context.metadata["parameter_position_mapping"]
    ...
    result_sql = result_context.current_expression.sql(dialect="duckdb")
...
def test_expression_simplifier_no_tracking_without_parameters() -> None:
    ...
    assert "parameter_position_mapping" not in result_context.metadata
...
def test_expression_simplifier_handles_complex_comparisons() -> None:
    ...
    context = SQLTransformContext(
        ...
        original_expression=expression,
        dialect="duckdb",
        parameters={"param_0": 10, "param_1": 20, "param_2": 30},
    )
