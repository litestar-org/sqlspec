...
def test_sql_translator_convert_to_dialect(
    ...
    # Create SQL object
    statement = SQL(input_sql, config=StatementConfig(parameter_config=DEFAULT_PARAMETER_CONFIG, dialect=from_dialect))
...
def test_sql_translator_handles_complex_queries() -> None:
    ...
    statement = SQL(complex_sql, config=StatementConfig(parameter_config=DEFAULT_PARAMETER_CONFIG, dialect="sqlite"))
...
def test_multiple_mixin_inheritance() -> None:
    ...
    statement = SQL(
        "SELECT * FROM users", config=StatementConfig(parameter_config=DEFAULT_PARAMETER_CONFIG, dialect="sqlite")
    )
    result = driver.convert_to_dialect(statement, to_dialect="postgres")
    assert isinstance(result, str)
    assert "SELECT" in result
