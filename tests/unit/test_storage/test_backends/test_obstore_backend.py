...
class TestObStoreBackend:
    ...
    def test_get_metadata_with_custom_metadata(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
    ...
    def test_get_signed_url_supported(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
        """Test signed URL generation when supported."""
        backend = backend_with_mock_store
        mock_store.sign_url = MagicMock(return_value="https://signed.url")
        
        url = backend.get_signed_url("file.txt", operation="read", expires_in=7200)
        
        assert url == "https://signed.url"
        mock_store.sign_url.assert_called_once_with("/data/file.txt", "read", 7200)

    def test_get_signed_url_not_supported(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
        """Test signed URL generation when not supported."""
        backend = backend_with_mock_store
        delattr(mock_store, "sign_url")
        
        with pytest.raises(StorageOperationFailedError, match="Failed to generate signed URL"):
    ...
    @pytest.mark.asyncio
    async def test_async_read_bytes(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_async_write_bytes(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_async_write_bytes_fallback(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
        """Test async write when method is missing."""
        backend = backend_with_mock_store
        delattr(mock_store, "put_async")
        
        # Without put_async method, it should raise AttributeError wrapped in StorageOperationFailedError
        with pytest.raises(StorageOperationFailedError, match="Failed to async write"):
            await backend.write_bytes_async("output.txt", b"data")

    @pytest.mark.asyncio
    async def test_async_read_text_with_bytes_object(self, backend_with_mock_store: ObStoreBackend) -> None:
        """Test async text reading with obstore's Bytes object."""
        backend = backend_with_mock_store
        
        # Mock a Bytes object
        bytes_obj = MagicMock()
        bytes_obj.to_bytes.return_value = b"text content"
        
        with patch.object(backend, "_read_bytes_async", return_value=bytes_obj):
            result = await backend.read_text_async("test.txt")
        
        assert result == "text content"
        bytes_obj.to_bytes.assert_called_once()

    @pytest.mark.asyncio
    async def test_async_list_objects(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
        """Test async listing objects."""
        backend = backend_with_mock_store
        
        # Mock async iterator
        async def mock_list_async(prefix: str):
            item1 = MagicMock()
            item1.path = "/data/file1.txt"
            item2 = MagicMock(spec=[])  # spec=[] prevents auto-creating attributes
            item2.key = "/data/file2.txt"  # Test key fallback
            for item in [item1, item2]:
                yield item
        
        mock_store.list_async = mock_list_async
    ...
    @pytest.mark.asyncio
    async def test_async_operations_with_native_support(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
        """Test various async operations with native support."""
        backend = backend_with_mock_store
        
        # Test exists_async
        mock_store.head_async = AsyncMock()
        assert await backend.exists_async("test.txt") is True
        
        # Test delete_async
        mock_store.delete_async = AsyncMock()
        await backend.delete_async("test.txt")
        mock_store.delete_async.assert_called_once()
        
        # Test copy_async
        mock_store.copy_async = AsyncMock()
        await backend.copy_async("src.txt", "dst.txt")
        mock_store.copy_async.assert_called_once()
        
        # Test move_async
        mock_store.rename_async = AsyncMock()
        await backend.move_async("old.txt", "new.txt")
        mock_store.rename_async.assert_called_once()

    @pytest.mark.asyncio
    async def test_async_operations_with_fallback(self, backend_with_mock_store: ObStoreBackend, mock_store: MagicMock) -> None:
        """Test async operations when methods are missing."""
        backend = backend_with_mock_store
        
        # Remove async methods to simulate missing functionality
        delattr(mock_store, "head_async")
        delattr(mock_store, "delete_async")
        
        # Without head_async method, exists_async should return False (exception handled)
        assert await backend.exists_async("test.txt") is False
        
        # Without delete_async method, delete_async should fail
        with pytest.raises(StorageOperationFailedError, match="Failed to async delete"):
            await backend.delete_async("test.txt")
        ...
        with patch("sqlspec.typing.OBSTORE_INSTALLED", True):
            ...
            with patch("obstore.store.from_url", return_value=mock_store):
                ...
                with caplog.at_level(logging.DEBUG):
                    ...
                    backend = ObStoreBackend("s3://test", instrumentation_config=config)
