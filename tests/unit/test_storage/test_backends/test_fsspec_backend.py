...
class TestFSSpecBackend:
    ...
    def backend_with_mock_fs(self, mock_fs: MagicMock) -> FSSpecBackend:
        ...
        with patch("sqlspec.typing.FSSPEC_INSTALLED", True):
            backend = FSSpecBackend(mock_fs, base_path="/base")
            return backend
    ...
    @pytest.mark.asyncio
    async def test_async_read_bytes_with_native_support(self, backend_with_mock_fs: FSSpecBackend, mock_fs: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_async_write_bytes_with_native_support(self, backend_with_mock_fs: FSSpecBackend, mock_fs: MagicMock) -> None:
    ...
    def test_from_config(self) -> None:
        ...
        with patch("sqlspec.typing.FSSPEC_INSTALLED", True):
            with patch("fsspec.filesystem") as mock_filesystem:
                ...
                config = {
                    "protocol": "s3",
                    "fs_config": {"key": "test", "secret": "test"},
                    "base_path": "/data"
                }
