...
# CSV data file paths
BLOGDB_PATH = Path(__file__).parent / "sample"
USERS_DATA_PATH = BLOGDB_PATH / "data/users_data.csv"
BLOGS_DATA_PATH = BLOGDB_PATH / "data/blogs_data.csv"


def create_user_blogs(db) -> tuple[str, ...]:
    assert db in {"sqlite", "pgsql", "mysql", "duckdb"}
    serial = "SERIAL" if db == "pgsql" else "INTEGER" if db in {"sqlite", "duckdb"} else "INTEGER auto_increment"
    ddl_statements = [
...
def drop_user_blogs(db) -> tuple[str, ...]:
    return (
...
def fill_user_blogs(cur, db) -> None:
        ...
        cur.executemany(
            f"""
               INSERT INTO users (
                    ...
                    firstname,
                    lastname
               ) VALUES ({param}, {param}, {param});""",  # noqa: S608
            users,
        )
        ...
        cur.executemany(
            f"""
                INSERT INTO blogs (
                    ...
                    content,
                    published
                ) VALUES ({param}, {param}, {param}, {param});""",  # noqa: S608
            blogs,
        )
