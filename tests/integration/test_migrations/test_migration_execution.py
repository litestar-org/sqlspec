...
class MockMigrationRunner(BaseMigrationRunner):
    ...
    def execute_upgrade(self, driver: Any, migration: dict[str, Any]) -> ExecutionResult:
        ...
        if sql:
            # Simulate execution
            self._executed_migrations.append({
...
class TestMigrationTrackingTableOperations:
    """Test migration tracking table operations."""
    ...
    def test_tracking_table_sql_generation(self) -> None:
    ...
    def test_current_version_sql_generation(self) -> None:
    ...
    def test_applied_migrations_sql_generation(self) -> None:
        ...
        assert "test_migrations" in applied_sql.sql
        assert "ORDER BY" in applied_sql.sql.upper()
        assert "version_num" in applied_sql.sql

    def test_record_migration_sql_generation(self) -> None:
    ...
    def test_remove_migration_sql_generation(self) -> None:
...
class TestMigrationExecutionWorkflow:
    """Test migration execution workflow."""

    @pytest.fixture
    def temp_workspace(self) -> Generator[Path, None, None]:
        """Create a temporary workspace for migration execution tests."""
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
    ...
    def test_single_migration_upgrade_execution(self, temp_workspace: Path) -> None:
...
"""
        ...
        with patch("sqlspec.migrations.base.run_") as mock_run:
            mock_run.return_value = ["CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL);"]
    ...
    def test_single_migration_downgrade_execution(self, temp_workspace: Path) -> None:
...
"""
    ...
    def test_multiple_migrations_execution_order(self, temp_workspace: Path) -> None:
        ...
        migrations = [
            ...
            ("0001_create_users.sql", "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, email TEXT);", "DROP TABLE users;"),
            ("0002_add_products.sql", "CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT, price REAL);", "DROP TABLE products;"),
        ]
...
"""
    ...
    def test_migration_with_no_downgrade(self, temp_workspace: Path) -> None:
...
"""
        ...
        with patch("sqlspec.migrations.base.run_") as mock_run:
            mock_run.return_value = ["CREATE TABLE irreversible_data AS SELECT DISTINCT column1, column2 FROM legacy_table;"]
...
class TestMigrationStateTracking:
    ...
    def test_migration_state_recording(self) -> None:
    ...
    def test_current_version_tracking(self) -> None:
    ...
    def test_migration_removal(self) -> None:
    ...
    def test_applied_migrations_ordering(self) -> None:
...
class TestMigrationErrorScenarios:
    ...
    @pytest.fixture
    def temp_workspace(self) -> Generator[Path, None, None]:
        """Create a temporary workspace for error scenario tests."""
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            migrations_dir = workspace / "migrations"
            migrations_dir.mkdir()
    ...
    def test_migration_execution_failure(self, temp_workspace: Path) -> None:
...
"""
    ...
    def test_missing_upgrade_migration(self, temp_workspace: Path) -> None:
...
"""
    ...
    def test_corrupted_migration_file(self, temp_workspace: Path) -> None:
...
"""
    ...
    def test_duplicate_version_detection(self, temp_workspace: Path) -> None:
