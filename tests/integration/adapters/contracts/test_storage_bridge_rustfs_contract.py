...
def test_sync_storage_bridge_rustfs_contract(
    sync_driver_case: DriverCaseContext, rustfs_service: "RustfsService", rustfs_bucket_name: str
) -> None:
    """Sync drivers round-trip a SELECT through RustFS object storage."""
    assert_sync_storage_bridge_rustfs_contract(
        sync_driver_case.driver, sync_driver_case.case, rustfs_service, rustfs_bucket_name
    )


async def test_async_storage_bridge_rustfs_contract(
    async_driver_case: DriverCaseContext, rustfs_service: "RustfsService", rustfs_bucket_name: str
) -> None:
    """Async drivers round-trip a SELECT through RustFS object storage."""
    await assert_async_storage_bridge_rustfs_contract(
        async_driver_case.driver, async_driver_case.case, rustfs_service, rustfs_bucket_name
    )
