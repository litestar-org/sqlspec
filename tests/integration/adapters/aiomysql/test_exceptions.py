"""Exception handling integration tests for aiomysql adapter."""
...
@pytest.fixture
async def aiomysql_exception_session(mysql_service: MySQLService) -> AsyncGenerator[AiomysqlDriver, None]:
    """Create an aiomysql session for exception testing."""
...
async def test_unique_violation(aiomysql_exception_session: AiomysqlDriver) -> None:
    """Test unique constraint violation raises UniqueViolationError."""
    await aiomysql_exception_session.execute_script("""
    ...
    await aiomysql_exception_session.execute(
    ...
    await aiomysql_exception_session.execute("DROP TABLE test_unique_constraint")


async def test_foreign_key_violation(aiomysql_exception_session: AiomysqlDriver) -> None:
    ...
    with pytest.raises(ForeignKeyViolationError) as exc_info:
        await aiomysql_exception_session.execute("INSERT INTO test_fk_child (parent_id) VALUES (%s)", (999,))
...
async def test_not_null_violation(aiomysql_exception_session: AiomysqlDriver) -> None:
    ...
    with pytest.raises(NotNullViolationError) as exc_info:
        await aiomysql_exception_session.execute("INSERT INTO test_not_null (id) VALUES (%s)", (1,))

    assert "cannot be null" in str(exc_info.value).lower() or any(
        code in str(exc_info.value) for code in ["1048", "1364"]
    )

    await aiomysql_exception_session.execute("DROP TABLE test_not_null")


async def test_check_violation(aiomysql_exception_session: AiomysqlDriver) -> None:
    ...
    with pytest.raises(CheckViolationError) as exc_info:
        await aiomysql_exception_session.execute("INSERT INTO test_check_constraint (age) VALUES (%s)", (15,))

    assert "check" in str(exc_info.value).lower() or "3819" in str(exc_info.value)

    await aiomysql_exception_session.execute("DROP TABLE test_check_constraint")


async def test_sql_parsing_error(aiomysql_exception_session: AiomysqlDriver) -> None:
    """Test syntax error raises SQLParsingError."""
    with pytest.raises(SQLParsingError) as exc_info:
        await aiomysql_exception_session.execute("SELCT * FROM nonexistent_table")
