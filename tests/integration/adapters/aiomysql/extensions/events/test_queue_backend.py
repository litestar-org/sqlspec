...
"""aiomysql integration tests for the EventChannel queue backend."""
...
@pytest.mark.mysql
async def test_aiomysql_event_channel_queue_fallback(mysql_service: MySQLService, tmp_path: Any) -> None:
    """aiomysql configs publish, consume, and ack events via the queue backend."""
    ...
    config = AiomysqlConfig(
        ...
        migration_config={
            ...
            "version_table_name": "ddl_migrations_aiomysql",
        },
        extension_config={"events": {"queue_table": "aiomysql_event_queue"}},
    )
    ...
    async with config.provide_session() as driver:
        row = await driver.select_one(
            "SELECT status FROM aiomysql_event_queue WHERE event_id = :event_id", {"event_id": event_id}
        )
...
@pytest.mark.mysql
async def test_aiomysql_event_channel_multiple_messages(mysql_service: MySQLService, tmp_path: Any) -> None:
    """aiomysql queue backend handles multiple messages correctly."""
    migrations = tmp_path / "migrations"
    migrations.mkdir()
...
@pytest.mark.mysql
async def test_aiomysql_event_channel_nack_redelivery(mysql_service: MySQLService, tmp_path: Any) -> None:
    """aiomysql queue backend redelivers nacked messages."""
    migrations = tmp_path / "migrations"
    migrations.mkdir()
    ...
    async with config.provide_session() as driver:
        row = await driver.select_one(
            "SELECT status, attempts FROM aiomysql_event_queue WHERE event_id = :event_id", {"event_id": event_id}
        )
