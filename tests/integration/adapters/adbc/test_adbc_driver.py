...
def test_adbc_postgresql_basic_crud(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    insert_result = adbc_postgresql_session.execute(
        "INSERT INTO test_table_adbc (name, value) VALUES ($1, $2)", ("test_name", 42)
    )
    assert isinstance(insert_result, SQLResult)
    assert insert_result.rows_affected in (-1, 0, 1)

    select_result = adbc_postgresql_session.execute(
        "SELECT name, value FROM test_table_adbc WHERE name = $1", ("test_name",)
    )
...
def test_adbc_sqlite_basic_crud(adbc_sqlite_session: AdbcDriver) -> None:
    ...
    insert_result = adbc_sqlite_session.execute(
        "INSERT INTO test_table_adbc (name, value) VALUES (?, ?)", ("test_name", 42)
    )
    assert isinstance(insert_result, SQLResult)
    assert insert_result.rows_affected in (-1, 0, 1)

    select_result = adbc_sqlite_session.execute(
        "SELECT name, value FROM test_table_adbc WHERE name = ?", ("test_name",)
    )
    assert isinstance(select_result, SQLResult)
    assert select_result.data is not None
    assert len(select_result.data) == 1
...
def test_adbc_duckdb_basic_crud(adbc_duckdb_session: AdbcDriver) -> None:
    ...
    insert_result = adbc_duckdb_session.execute(
        "INSERT INTO test_table_adbc (id, name, value) VALUES (?, ?, ?)", (1, "test_name", 42)
    )
    assert isinstance(insert_result, SQLResult)
    assert insert_result.rows_affected in (-1, 0, 1)
...
def test_adbc_postgresql_parameter_styles(
    ...
    adbc_postgresql_session.execute("INSERT INTO test_table_adbc (name) VALUES ($1)", ("test_value",))

    if style == "tuple_binds":
        sql = "SELECT name FROM test_table_adbc WHERE name = $1"
    else:
        sql = "SELECT name FROM test_table_adbc WHERE name = $1"
...
def test_adbc_postgresql_execute_many(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    result = adbc_postgresql_session.execute_many(
        "INSERT INTO test_table_adbc (name, value) VALUES ($1, $2)", parameters_list
    )
    assert isinstance(result, SQLResult)
...
def test_adbc_postgresql_execute_script(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    script = """
        INSERT INTO test_table_adbc (name, value) VALUES ('script_test1', 999);
        INSERT INTO test_table_adbc (name, value) VALUES ('script_test2', 888);
        UPDATE test_table_adbc SET value = 1000 WHERE name = 'script_test1';
    """
    ...
    select_result = adbc_postgresql_session.execute(
        "SELECT name, value FROM test_table_adbc WHERE name LIKE 'script_test%' ORDER BY name"
    )
    assert isinstance(select_result, SQLResult)
    assert select_result.data is not None
...
def test_adbc_postgresql_statement_stack_sequential(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    stack = (
        StatementStack()
        .push_execute("INSERT INTO test_table_adbc (id, name, value) VALUES ($1, $2, $3)", (1, "adbc-stack-one", 10))
        .push_execute("INSERT INTO test_table_adbc (id, name, value) VALUES ($1, $2, $3)", (2, "adbc-stack-two", 20))
        .push_execute("SELECT COUNT(*) AS total FROM test_table_adbc WHERE name LIKE $1", ("adbc-stack-%",))
    )
...
def test_adbc_postgresql_result_methods(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    adbc_postgresql_session.execute_many(
        "INSERT INTO test_table_adbc (name, value) VALUES ($1, $2)", [("result1", 10), ("result2", 20), ("result3", 30)]
    )

    result = adbc_postgresql_session.execute("SELECT * FROM test_table_adbc ORDER BY name")
    assert isinstance(result, SQLResult)
    ...
    empty_result = adbc_postgresql_session.execute("SELECT * FROM test_table_adbc WHERE name = $1", ("nonexistent",))
    assert isinstance(empty_result, SQLResult)
...
def test_adbc_postgresql_error_handling(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    adbc_postgresql_session.execute_script("DROP TABLE IF EXISTS test_table_adbc")
    ...
    adbc_postgresql_session.execute("INSERT INTO test_table_adbc (name, value) VALUES ($1, $2)", ("unique_test", 1))

    with pytest.raises(Exception):
        adbc_postgresql_session.execute("INSERT INTO test_table_adbc (name, value) VALUES ($1, $2)", ("unique_test", 2))

    with pytest.raises(Exception):
        adbc_postgresql_session.execute("SELECT nonexistent_column FROM test_table_adbc")
...
def test_adbc_postgresql_data_types(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    adbc_postgresql_session.execute_script("""
        CREATE TABLE adbc_data_types_test_adbc (
            id SERIAL PRIMARY KEY,
            text_col TEXT,
            integer_col INTEGER,
    ...
    adbc_postgresql_session.execute(
        """
        INSERT INTO adbc_data_types_test_adbc (
    ...
    select_result = adbc_postgresql_session.execute(
        "SELECT text_col, integer_col, numeric_col, boolean_col, array_col FROM adbc_data_types_test_adbc"
    )
    assert isinstance(select_result, SQLResult)
    assert select_result.data is not None
    assert len(select_result.data) == 1
    ...
    adbc_postgresql_session.execute_script("DROP TABLE adbc_data_types_test_adbc")
...
def test_adbc_postgresql_performance_bulk_operations(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    result = adbc_postgresql_session.execute_many(
        "INSERT INTO test_table_adbc (name, value) VALUES ($1, $2)", bulk_data
    )
    assert isinstance(result, SQLResult)
    assert result.rows_affected == 100

    select_result = adbc_postgresql_session.execute(
        "SELECT COUNT(*) as count FROM test_table_adbc WHERE name LIKE 'bulk_user_%'"
    )
    assert isinstance(select_result, SQLResult)
    assert select_result.data is not None
...
def test_adbc_multiple_backends_consistency(adbc_sqlite_session: AdbcDriver) -> None:
    ...
    adbc_sqlite_session.execute_many("INSERT INTO test_table_adbc (name, value) VALUES (?, ?)", test_data)

    result = adbc_sqlite_session.execute("SELECT name, value FROM test_table_adbc ORDER BY name")
    assert isinstance(result, SQLResult)
    assert result.data is not None
...
def test_adbc_for_update_generates_sql(adbc_sqlite_session: AdbcDriver) -> None:
    ...
    # Setup test table
    adbc_sqlite_session.execute_script("DROP TABLE IF EXISTS test_table_adbc")
