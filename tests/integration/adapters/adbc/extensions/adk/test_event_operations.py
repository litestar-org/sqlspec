...
async def test_event_with_all_fields(adbc_store: Any, session_fixture: Any) -> None:
    """Test creating event with all optional fields stored in event_data."""
    event_record: EventRecord = {
        "session_id": session_fixture["session_id"],
    ...
    # Everything else is in event_data
    event_data = (
        json.loads(events[0]["event_data"]) if isinstance(events[0]["event_data"], str) else events[0]["event_data"]
    )
...
async def test_event_data_fields(adbc_store: Any, session_fixture: Any) -> None:
    """Test event JSON field serialization and deserialization via event_data."""
...
async def test_event_ordering(adbc_store: Any, session_fixture: Any) -> None:
    ...
    ev1: EventRecord = {
        ...
        "author": "",
        "timestamp": datetime.now(timezone.utc),
        "event_data": {"id": "event-1", "app_name": session_fixture["app_name"], "user_id": session_fixture["user_id"]},
    }
    ...
    ev2: EventRecord = {
        ...
        "author": "",
        "timestamp": datetime.now(timezone.utc),
        "event_data": {"id": "event-2", "app_name": session_fixture["app_name"], "user_id": session_fixture["user_id"]},
    }
    ...
    ev3: EventRecord = {
        ...
        "author": "",
        "timestamp": datetime.now(timezone.utc),
        "event_data": {"id": "event-3", "app_name": session_fixture["app_name"], "user_id": session_fixture["user_id"]},
    }
...
async def test_event_with_large_content(adbc_store: Any, session_fixture: Any) -> None:
    """Test creating event with large content in event_data."""
...
async def test_get_events_applies_after_timestamp_and_limit(adbc_store: Any, session_fixture: Any) -> None:
    ...
    filtered_event = filtered_events[0]["event_data"]
