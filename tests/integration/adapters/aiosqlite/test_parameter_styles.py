...
async def test_aiosqlite_named_colon_style_insert_and_select() -> None:
    """Named-colon parameters must work end-to-end via the sqlspec pipeline."""
    from sqlspec.core import SQL

    config = AiosqliteConfig(connection_config={"database": ":memory:"})

    async with config.provide_session() as driver:
        await driver.execute("CREATE TABLE test_named_colon (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)")

        result = await driver.execute(
            "INSERT INTO test_named_colon (id, name, value) VALUES (:id, :name, :value)",
        ...
        select_result = await driver.execute(
            SQL("SELECT name, value FROM test_named_colon WHERE name = :target", target="alice")
        )
        assert isinstance(select_result, SQLResult)
        row = select_result.get_data()[0]
...
async def test_aiosqlite_named_colon_style_execute_many() -> None:
    """Named-colon parameters must work with execute_many."""
    config = AiosqliteConfig(connection_config={"database": ":memory:"})

    async with config.provide_session() as driver:
        await driver.execute("CREATE TABLE test_named_colon_many (id INTEGER PRIMARY KEY, name TEXT)")

        params = [{"id": 1, "name": "alpha"}, {"id": 2, "name": "beta"}, {"id": 3, "name": "gamma"}]
        result = await driver.execute_many("INSERT INTO test_named_colon_many (id, name) VALUES (:id, :name)", params)
        assert isinstance(result, SQLResult)
        assert result.rows_affected == 3

        count_result = await driver.execute("SELECT COUNT(*) as cnt FROM test_named_colon_many")
        assert isinstance(count_result, SQLResult)
        assert count_result.get_data()[0]["cnt"] == 3
