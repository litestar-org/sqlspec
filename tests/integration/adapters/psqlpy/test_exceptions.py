...
async def test_unique_violation(psqlpy_exception_session: PsqlpyDriver) -> None:
    ...
    await psqlpy_exception_session.execute_script("""
        DROP TABLE IF EXISTS test_unique_constraint_psqlpy;
        CREATE TABLE test_unique_constraint_psqlpy (
    ...
    await psqlpy_exception_session.execute(
        "INSERT INTO test_unique_constraint_psqlpy (email) VALUES ($1)", ("test@example.com",)
    )
    ...
    await psqlpy_exception_session.execute("DROP TABLE test_unique_constraint_psqlpy")
...
async def test_foreign_key_violation(psqlpy_exception_session: PsqlpyDriver) -> None:
    ...
    await psqlpy_exception_session.execute_script("""
        DROP TABLE IF EXISTS test_fk_child_psqlpy CASCADE;
        DROP TABLE IF EXISTS test_fk_parent_psqlpy CASCADE;
        CREATE TABLE test_fk_parent_psqlpy (
            id SERIAL PRIMARY KEY,
            name VARCHAR(100)
        );
        CREATE TABLE test_fk_child_psqlpy (
            id SERIAL PRIMARY KEY,
            parent_id INTEGER NOT NULL,
            FOREIGN KEY (parent_id) REFERENCES test_fk_parent_psqlpy(id)
        );
    """)

    with pytest.raises(ForeignKeyViolationError) as exc_info:
        await psqlpy_exception_session.execute("INSERT INTO test_fk_child_psqlpy (parent_id) VALUES ($1)", (999,))
...
async def test_not_null_violation(psqlpy_exception_session: PsqlpyDriver) -> None:
    ...
    await psqlpy_exception_session.execute_script("""
        DROP TABLE IF EXISTS test_not_null_psqlpy;
        CREATE TABLE test_not_null_psqlpy (
            id SERIAL PRIMARY KEY,
    ...
    with pytest.raises(NotNullViolationError) as exc_info:
        await psqlpy_exception_session.execute("INSERT INTO test_not_null_psqlpy (id) VALUES ($1)", (1,))

    assert ("not" in str(exc_info.value).lower() and "null" in str(exc_info.value).lower()) or "23502" in str(
        exc_info.value
    )

    await psqlpy_exception_session.execute("DROP TABLE test_not_null_psqlpy")
...
async def test_check_violation(psqlpy_exception_session: PsqlpyDriver) -> None:
    ...
    await psqlpy_exception_session.execute_script("""
        DROP TABLE IF EXISTS test_check_constraint_psqlpy;
        CREATE TABLE test_check_constraint_psqlpy (
            id SERIAL PRIMARY KEY,
    ...
    with pytest.raises(CheckViolationError) as exc_info:
        await psqlpy_exception_session.execute("INSERT INTO test_check_constraint_psqlpy (age) VALUES ($1)", (15,))

    assert "check" in str(exc_info.value).lower() or "23514" in str(exc_info.value)

    await psqlpy_exception_session.execute("DROP TABLE test_check_constraint_psqlpy")
