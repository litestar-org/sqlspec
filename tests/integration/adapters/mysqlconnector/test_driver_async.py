...
async def test_mysqlconnector_async_basic_crud(mysqlconnector_async_driver: MysqlConnectorAsyncDriver) -> None:
    ...
    insert_result = await driver.execute(
        "INSERT INTO test_table_mysqlconnector_async (name, value) VALUES (?, ?)", ("test_user", 42)
    )
    assert insert_result.num_rows == 1

    select_result = await driver.execute("SELECT * FROM test_table_mysqlconnector_async WHERE name = ?", ("test_user",))
    ...
    update_result = await driver.execute(
        "UPDATE test_table_mysqlconnector_async SET value = ? WHERE name = ?", (100, "test_user")
    )
    assert update_result.num_rows == 1

    updated_result = await driver.execute(
        "SELECT value FROM test_table_mysqlconnector_async WHERE name = ?", ("test_user",)
    )
    assert updated_result.get_data()[0]["value"] == 100

    delete_result = await driver.execute("DELETE FROM test_table_mysqlconnector_async WHERE name = ?", ("test_user",))
    assert delete_result.num_rows == 1

    verify_result = await driver.execute(
        "SELECT COUNT(*) as count FROM test_table_mysqlconnector_async WHERE name = ?", ("test_user",)
    )
...
async def test_mysqlconnector_async_execute_many(mysqlconnector_async_driver: MysqlConnectorAsyncDriver) -> None:
    ...
    result = await driver.execute_many("INSERT INTO test_table_mysqlconnector_async (name, value) VALUES (?, ?)", data)
    assert result.num_rows == 3

    select_result = await driver.execute(
        "SELECT name, value FROM test_table_mysqlconnector_async WHERE name LIKE ? ORDER BY name", ("batch_user_%",)
    )
...
async def test_mysqlconnector_async_execute_script(mysqlconnector_async_driver: MysqlConnectorAsyncDriver) -> None:
    ...
    script = """
        INSERT INTO test_table_mysqlconnector_async (name, value) VALUES ('script_user_1', 1000);
        INSERT INTO test_table_mysqlconnector_async (name, value) VALUES ('script_user_2', 2000);
        UPDATE test_table_mysqlconnector_async SET value = value * 2 WHERE name LIKE 'script_user_%';
    """
    ...
    select_result = await driver.execute(
        "SELECT name, value FROM test_table_mysqlconnector_async WHERE name LIKE ? ORDER BY name", ("script_user_%",)
    )
...
async def test_mysqlconnector_async_data_types(mysqlconnector_async_driver: MysqlConnectorAsyncDriver) -> None:
    ...
    await driver.execute_script("""
        CREATE TABLE IF NOT EXISTS data_types_test_mysqlconnector_async (
            id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    result = await driver.execute(
        """INSERT INTO data_types_test_mysqlconnector_async
    ...
    select_result = await driver.execute(
        "SELECT * FROM data_types_test_mysqlconnector_async WHERE text_col = ? AND int_col = ?", ("test_string", 42)
    )
    row = select_result.get_data()[0]
...
async def test_mysqlconnector_async_transactions(mysqlconnector_async_driver: MysqlConnectorAsyncDriver) -> None:
    ...
    await driver.begin()
    await driver.execute("INSERT INTO test_table_mysqlconnector_async (name, value) VALUES (?, ?)", ("tx_user_1", 100))
    await driver.commit()

    result = await driver.execute(
        "SELECT COUNT(*) as count FROM test_table_mysqlconnector_async WHERE name = ?", ("tx_user_1",)
    )
    assert result.get_data()[0]["count"] == 1

    await driver.begin()
    await driver.execute("INSERT INTO test_table_mysqlconnector_async (name, value) VALUES (?, ?)", ("tx_user_2", 200))
    await driver.rollback()

    result = await driver.execute(
        "SELECT COUNT(*) as count FROM test_table_mysqlconnector_async WHERE name = ?", ("tx_user_2",)
    )
...
async def test_mysqlconnector_async_sql_object_execution(
    ...
    sql_obj = SQL("INSERT INTO test_table_mysqlconnector_async (name, value) VALUES (?, ?)", "sql_obj_test", 999)
    result = await driver.execute(sql_obj)
    assert isinstance(result, SQLResult)
    assert result.num_rows == 1

    verify_result = await driver.execute(
        "SELECT name, value FROM test_table_mysqlconnector_async WHERE name = ?", ("sql_obj_test",)
    )
    assert verify_result.get_data()[0]["name"] == "sql_obj_test"
    assert verify_result.get_data()[0]["value"] == 999

    select_sql = SQL("SELECT * FROM test_table_mysqlconnector_async WHERE value > ?", 500)
...
async def test_mysqlconnector_async_on_connection_create_hook(mysql_service: "MySQLService") -> None:
