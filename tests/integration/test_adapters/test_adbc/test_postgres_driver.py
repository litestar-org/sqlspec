...
def test_connection(postgres_service: PostgresService) -> None:
    ...
    with config.provide_session() as session:
        ...
        # PostgreSQL drivers may return strings for numeric literals
...
@pytest.mark.xdist_group("postgres")
def test_parameter_styles(adbc_postgresql_session: AdbcDriver, parameters: Any, style: ParamStyle) -> None:
...
def test_execute_many(adbc_postgresql_session: AdbcDriver) -> None:
    ...
    result = adbc_postgresql_session.execute_many(
        SQL("INSERT INTO test_table (name, value) VALUES ($1, $2)"), parameters_list
    )
    assert isinstance(result, SQLResult)
