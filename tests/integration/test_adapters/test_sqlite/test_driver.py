...
def test_sqlite_data_types(sqlite_session: SqliteDriver) -> None:
    ...
    sqlite_session.execute_script("""
        CREATE TABLE sqlite_data_types_test (
    ...
    insert_result = sqlite_session.execute(
        "INSERT INTO sqlite_data_types_test (text_col, integer_col, real_col, blob_col, null_col) VALUES (?, ?, ?, ?, ?)",
        test_data,
    )
    assert isinstance(insert_result, SQLResult)
    assert insert_result.rows_affected == 1
    ...
    select_result = sqlite_session.execute(
        "SELECT text_col, integer_col, real_col, blob_col, null_col FROM sqlite_data_types_test"
    )
    assert isinstance(select_result, SQLResult)
    assert select_result.data is not None
    assert len(select_result.data) == 1
