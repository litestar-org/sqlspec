...
@pytest.fixture
def sqlite_session(sqlite_sync_config: SqliteConfig) -> Generator[SqliteDriver, None, None]:
    """Create a sqlite session with test table."""
    with sqlite_sync_config.provide_session() as session:
...
def test_explain_query_plan_select(sqlite_session: SqliteDriver) -> None:
    """Test EXPLAIN QUERY PLAN on SELECT statement."""
    explain_stmt = Explain("SELECT * FROM explain_test", dialect="sqlite")
    result = sqlite_session.execute(explain_stmt.build())

    assert isinstance(result, SQLResult)
    assert result.data is not None


def test_explain_query_plan_with_where(sqlite_session: SqliteDriver) -> None:
...
def test_explain_query_plan_with_join(sqlite_session: SqliteDriver) -> None:
    """Test EXPLAIN QUERY PLAN with JOIN."""
    sqlite_session.execute_script("DROP TABLE IF EXISTS explain_test2")
...
def test_explain_from_query_builder(sqlite_session: SqliteDriver) -> None:
...
def test_explain_from_sql_factory(sqlite_session: SqliteDriver) -> None:
...
def test_explain_from_sql_object(sqlite_session: SqliteDriver) -> None:
    """Test SQL.explain() method."""
    stmt = SQL("SELECT * FROM explain_test")
    explain_stmt = stmt.explain()
    result = sqlite_session.execute(explain_stmt)

    assert isinstance(result, SQLResult)
    assert result.data is not None


def test_explain_insert(sqlite_session: SqliteDriver) -> None:
...
def test_explain_update(sqlite_session: SqliteDriver) -> None:
...
def test_explain_delete(sqlite_session: SqliteDriver) -> None:
...
def test_explain_subquery(sqlite_session: SqliteDriver) -> None:
