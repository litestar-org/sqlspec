...
@pytest.fixture
async def aiosqlite_session(aiosqlite_config: AiosqliteConfig) -> AsyncGenerator[AiosqliteDriver, None]:
...
async def test_explain_query_plan_select(aiosqlite_session: AiosqliteDriver) -> None:
    """Test EXPLAIN QUERY PLAN on SELECT statement."""
    explain_stmt = Explain("SELECT * FROM explain_test", dialect="sqlite")
    result = await aiosqlite_session.execute(explain_stmt.build())

    assert isinstance(result, SQLResult)
    assert result.data is not None


async def test_explain_query_plan_with_where(aiosqlite_session: AiosqliteDriver) -> None:
...
async def test_explain_from_query_builder(aiosqlite_session: AiosqliteDriver) -> None:
...
async def test_explain_from_sql_factory(aiosqlite_session: AiosqliteDriver) -> None:
...
async def test_explain_from_sql_object(aiosqlite_session: AiosqliteDriver) -> None:
    """Test SQL.explain() method."""
    stmt = SQL("SELECT * FROM explain_test")
    explain_stmt = stmt.explain()
    result = await aiosqlite_session.execute(explain_stmt)

    assert isinstance(result, SQLResult)
    assert result.data is not None


async def test_explain_insert(aiosqlite_session: AiosqliteDriver) -> None:
...
async def test_explain_update(aiosqlite_session: AiosqliteDriver) -> None:
...
async def test_explain_delete(aiosqlite_session: AiosqliteDriver) -> None:
