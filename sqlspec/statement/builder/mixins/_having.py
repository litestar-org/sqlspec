...
class HavingClauseMixin:
    ...
    def having(self, condition: Union[str, exp.Expression]) -> Self:
        ...
        if self._expression is None:
            self._expression = exp.Select()  # pyright: ignore  # type: ignore[attr-defined]
        if not isinstance(self._expression, exp.Select):  # type: ignore[attr-defined]
        ...
        self._expression = self._expression.having(having_expr, copy=False)  # pyright: ignore  # type: ignore[attr-defined]
        return self
