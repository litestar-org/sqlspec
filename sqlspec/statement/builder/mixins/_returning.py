...
class ReturningClauseMixin:
    ...
    def returning(self, *columns: Union[str, exp.Expression]) -> Self:
        ...
        """
        if self._expression is None:  # type: ignore[attr-defined]
            msg = "Cannot add RETURNING: expression is not initialized."
            raise SQLBuilderError(msg)  # type: ignore[attr-defined]
        valid_types = (exp.Insert, exp.Update, exp.Delete)
        if not isinstance(self._expression, valid_types):  # type: ignore[attr-defined]
            msg = f"RETURNING not supported for {type(self._expression).__name__}."  # type: ignore[attr-defined]
            raise SQLBuilderError(msg)
        returning_exprs = [exp.column(c) if isinstance(c, str) else c for c in columns]
        self._expression.set("returning", exp.Returning(expressions=returning_exprs))  # type: ignore[attr-defined]
        return self
