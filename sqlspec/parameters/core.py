    ...
    def _adjust_params_for_style(
        ...
        if not expects_dict and params_are_dict:
            ...
            return [
                ...
                else params[f"param_{p_info.ordinal}"]
                if f"param_{p_info.ordinal}" in params
                else params_values[p_info.ordinal]
                if p_info.ordinal < len(params_values)
                else None
                for p_info in param_info
            ]

        if expects_dict and params_are_sequence:
            return {p_info.name or f"param_{i}": value for i, (p_info, value) in enumerate(zip(param_info, params))}
