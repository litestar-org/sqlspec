...
class Db2AsyncExceptionHandler(BaseAsyncExceptionHandler):
    """Async context manager that maps ``ibm_db_dbi`` errors to SQLSpec exceptions.
...
class Db2AsyncStreamSource:
    """Native async Db2 chunk source backed by ``AsyncCursor.fetchmany()``."""

    __slots__ = ("_chunk_size", "_column_names", "_cursor_manager", "_driver", "_parameters", "_sql")

    def __init__(self, driver: "Db2AsyncDriver", sql: str, parameters: Any, chunk_size: int) -> None:
        self._driver = driver
        self._sql = sql
        self._parameters = parameters
        self._chunk_size = chunk_size
        self._cursor_manager: Db2AsyncCursor | None = None
        self._column_names: list[str] | None = None
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            await self._driver._run_with_exception_handler(
                handler, cursor.execute, self._sql, normalize_execute_parameters(self._parameters)
            )
            self._driver._check_pending_exception(handler)
        except BaseException:
            with contextlib.suppress(Exception):
...
class Db2AsyncDriver(AsyncDriverAdapterBase):
    ...
    def __init__(
        ...
        super().__init__(connection=connection, statement_config=statement_config, driver_features=driver_features)
        self._data_dictionary: Db2AsyncDataDictionary | None = None
        self._column_name_cache: dict[int, tuple[Any, list[str]]] = {}
        self._lowercase_columns = bool(self.driver_features.get("enable_lowercase_column_names", True))
        self._transaction_active = False
        self._restore_autocommit = False
    ...
    async def dispatch_execute_many(self, cursor: Any, statement: "SQL") -> "ExecutionResult":
        ...
        prepared_parameters = normalize_execute_many_parameters(prepared_parameters)
        await cursor.executemany(sql, cast("Sequence[Any]", prepared_parameters))
    ...
    async def _restore_connection_autocommit(self) -> None:
        """Switch autocommit back on when ``begin()`` turned it off.
    ...
    def with_cursor(self, connection: Any) -> "Db2AsyncCursor":
        return Db2AsyncCursor(connection)

    def handle_database_exceptions(self) -> "Db2AsyncExceptionHandler":
        return Db2AsyncExceptionHandler()
    ...
    def dispatch_select_stream(self, statement: "SQL", chunk_size: int) -> "AsyncRowStream[dict[str, Any]] | None":
        ...
        if not statement.returns_rows():
            return None
        sql, prepared_parameters = self._compiled_sql(statement, self.statement_config)
        return AsyncRowStream(Db2AsyncStreamSource(self, sql, prepared_parameters, chunk_size))
    ...
    def data_dictionary(self) -> "Db2AsyncDataDictionary":
        ...
        """
        if self._data_dictionary is None:
            self._data_dictionary = Db2AsyncDataDictionary()
        return self._data_dictionary
