...
class PymssqlADKConfig(ADKConfig):
    """pymssql ADK extension settings."""
...
class PymssqlSyncADKStore(BaseSyncADKStore["PymssqlConfig"]):
    ...
    def create_session(
        ...
        VALUES (%s, %s, %s{owner_param}, %s, SYSUTCDATETIME(), SYSUTCDATETIME())
        """
        params: tuple[Any, ...]
    ...
    def update_session_state(self, app_name: str, user_id: str, session_id: str, state: "dict[str, Any]") -> None:
        ...
        self._execute(
            f"""
            UPDATE {_table_ref(self._session_table)}
            SET state = %s, update_time = SYSUTCDATETIME()
            WHERE app_name = %s AND user_id = %s AND id = %s
            """,
    ...
    def delete_session(self, app_name: str, user_id: str, session_id: str) -> None:
        ...
        self._execute(
            f"DELETE FROM {_table_ref(self._session_table)} WHERE app_name = %s AND user_id = %s AND id = %s",
            (app_name, user_id, session_id),
            commit=True,
        )
...
class PymssqlADKMemoryStore(BaseSyncADKMemoryStore["PymssqlConfig"]):
    """SQL Server ADK memory store using pymssql."""
...
def _get_upsert_state_sql(table: str, key_columns: "tuple[str, ...]", key_params: "tuple[str, ...]") -> str:
    ...
    source_columns = f"{source_columns}, %s AS state"
