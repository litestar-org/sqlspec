...
class AsyncmyADKConfig(ADKConfig):
...
class AsyncmyADKStore(BaseAsyncADKStore["AsyncmyConfig"]):
    ...
    async def upsert_app_state(self, app_name: str, state: "dict[str, Any]") -> None:
        ...
        async with self._config.provide_connection() as conn, conn.cursor() as cursor:
            await cursor.execute(_mysql_upsert_app_state_sql(self._app_state_table), (app_name, to_json(state)))
            await conn.commit()
    ...
    async def upsert_user_state(self, app_name: str, user_id: str, state: "dict[str, Any]") -> None:
        ...
        async with self._config.provide_connection() as conn, conn.cursor() as cursor:
            await cursor.execute(
                _mysql_upsert_user_state_sql(self._user_state_table), (app_name, user_id, to_json(state))
            )
            await conn.commit()
    ...
    async def _get_create_sessions_table_sql(self) -> str:
        ...
        adk_config = _get_asyncmy_adk_config(self._config)
    ...
    async def _get_create_events_table_sql(self) -> str:
        ...
        return _mysql_events_ddl(self._events_table, self._session_table, _get_asyncmy_adk_config(self._config))
...
def _get_asyncmy_adk_config(config: Any) -> AsyncmyADKConfig:
    """Return asyncmy ADK extension settings from ``extension_config["adk"]``."""
    ...
    if not isinstance(adk_config, dict):
        return {}
    return cast("AsyncmyADKConfig", adk_config)
