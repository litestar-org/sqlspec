...
class DuckDBAdapter(GenericAdapter):
    ...
    def insert_returning(self, conn, statement_name, sql, parameters):  # pragma: no cover
    ...
    def select(self, conn, statement_name: str, sql: str, parameters, record_class=None):
    ...
    def select_one(self, conn, statement_name, sql, parameters, record_class=None):
        cur = self._cursor(conn)
        try:
            cur.execute(sql, parameters)
            result = cur.fetchone()
