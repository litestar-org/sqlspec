...
class SpannerExceptionHandler:
    ...
    def _map_spanner_exception(self, e: Any) -> None:
        if isinstance(e, api_exceptions.AlreadyExists):
            raise UniqueViolationError(f"Spanner resource already exists: {e}") from e
        elif isinstance(e, api_exceptions.NotFound):
            raise NotFoundError(f"Spanner resource not found: {e}") from e
        elif isinstance(e, api_exceptions.InvalidArgument):
            raise SQLParsingError(f"Invalid Spanner query/argument: {e}") from e
        elif isinstance(e, api_exceptions.PermissionDenied):
            raise DatabaseConnectionError(f"Spanner permission denied: {e}") from e
        elif isinstance(e, (api_exceptions.ServiceUnavailable, api_exceptions.TooManyRequests)):
            raise OperationalError(f"Spanner service unavailable/rate limited: {e}") from e
        else:
            raise SQLSpecError(f"Spanner error: {e}") from e


class SpannerSyncDriver(
    ...
    def _execute_script(self, cursor: SpannerConnection, statement: "SQL") -> ExecutionResult:
        ...
        return self.create_execution_result(cursor, statement_count=count, successful_statements=count, is_script_result=True)
    ...
    def _execute_many(self, cursor: SpannerConnection, statement: "SQL") -> ExecutionResult:
        ...
        compiled_sql, _ = self._get_compiled_sql(
            statement.replace(parameters=first_params), 
            self.statement_config
        )
        
        batch_inputs = []
        for p in statement.parameters:
             _, processed_params = self._get_compiled_sql(
                 statement.replace(parameters=p),
                 self.statement_config
             )
...
spanner_statement_config = build_statement_config_from_profile(
    _SPANNER_PROFILE, 
    statement_overrides={"dialect": "spanner"}, 
