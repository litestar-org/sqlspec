...
class SpannerSyncADKStore(BaseSyncADKStore[SpannerSyncConfig]):
    ...
    def delete_session(self, session_id: str) -> None:
        delete_events_sql = f"DELETE FROM {self._events_table} WHERE session_id = @session_id"
        delete_session_sql = f"DELETE FROM {self._session_table} WHERE id = @session_id"
...
CREATE TABLE {self._events_table} (
  ...
  INTERLEAVE IN PARENT {self._session_table} ON DELETE CASCADE
"""
