...
from sqlspec.utils.type_guards import has_parameter_builder, has_sqlglot_expression
...
def parse_column_expression(column_input: Union[str, exp.Expression, Any]) -> exp.Expression:
    ...
    # Handle our custom Column objects with sqlglot_expression property
    if has_sqlglot_expression(column_input):
        expr = column_input.sqlglot_expression
        if expr is not None:
            return cast("exp.Expression", expr)
...
def parse_condition_expression(
    condition_input: Union[str, exp.Expression, tuple[str, Any], Any], builder: "Any" = None
) -> exp.Expression:
    ...
    try:
        ...
        # Common parsing errors include ValueError for invalid syntax,
        # AttributeError for missing parser methods, TypeError for invalid input
        try:
            ...
            # This is expected for complex or database-specific syntax
            pass
