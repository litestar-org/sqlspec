"""Provider functions for SQLSpec Flask integration."""
...
def create_pool_provider(
    ...
    def sync_provide_pool() -> "PoolT":
        """Synchronous wrapper for pool provider."""
        portal = PortalProvider()
        if not portal.is_running:
            portal.start()
        return portal.call(provide_pool)

    # Return sync version for Flask
    return sync_provide_pool  # type: ignore[return-value]
...
def create_connection_provider(
    ...
    def sync_provide_connection() -> "ConnectionT":
        """Synchronous wrapper for connection provider."""

        # Use the portal to get a connection synchronously
        async def get_connection() -> "ConnectionT":
            async for conn in provide_connection():
                return conn
            return None
        ...
        if not portal.is_running:
            portal.start()
        return portal.call(get_connection)

    # Return sync version for Flask
    return sync_provide_connection  # type: ignore[return-value]
...
def create_session_provider(
    ...
    def sync_provide_session(connection: "ConnectionT") -> "DriverT":
        """Synchronous wrapper for session provider."""

        async def get_session() -> "DriverT":
            async for session in provide_session(connection):
                return session
            return None
        ...
        if not portal.is_running:
            portal.start()
        return portal.call(get_session)

    # Return sync version for Flask
    return sync_provide_session  # type: ignore[return-value]
