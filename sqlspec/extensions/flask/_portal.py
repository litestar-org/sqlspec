"""Portal provider for calling async functions from synchronous Flask routes.

Provides a background thread with an event loop to execute async database operations
in sync Flask routes. Based on the portal pattern from Advanced Alchemy.
"""
...
__all__ = ("Portal", "PortalProvider")

logger = get_logger("extensions.flask.portal")
...
class PortalProvider:
    ...
    from synchronous Flask routes.
    """

    def __init__(self) -> None:
        """Initialize the PortalProvider."""
        self._request_queue: "queue.Queue[tuple[Callable[..., Coroutine[Any, Any, Any]], tuple[Any, ...], dict[str, Any], queue.Queue[tuple[Any | None, Exception | None]]]]" = (
            queue.Queue()
        )
    ...
    def _process_request(self) -> None:
        ...
        if not self._request_queue.empty():
            ...
            future.add_done_callback(
                functools.partial(self._handle_future_result, local_result_queue=local_result_queue),
            )

    @staticmethod
    def _handle_future_result(
        future: "asyncio.Future[Any]",
        local_result_queue: "queue.Queue[tuple[Any | None, Exception | None]]",
    ) -> None:
