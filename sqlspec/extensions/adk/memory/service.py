...
class SQLSpecMemoryService(BaseMemoryService):
    ...
    ADK BaseMemoryService defines the memory write/read contract:
    - add_session_to_memory(session) - Ingests session into memory (returns void)
    - add_events_to_memory(...) - Ingests explicit event deltas
    - add_memory(...) - Persists explicit MemoryEntry objects
        ...
        *,
        ...
        for event in events:
            ...
            if record is not None:
                if metadata_dict:
                    merged_metadata = dict(metadata_dict)
                    if record["metadata_json"]:
                        merged_metadata.update(record["metadata_json"])
